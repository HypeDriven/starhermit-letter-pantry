...
Letter Pantry is a game in which players rearrange a small letter set into all valid target words and bonus words. Its signature setting is a playful pantry where letter biscuits sit on a tray. The product should feel immediately understandable, responsive within one input, and polished enough that the board or playfield itself is the visual hero. Sessions should begin quickly, make the next useful action obvious without solving the game for the player, and end with a clear explanation of score and progress.
...
Rearrange a small letter set into all valid target words and bonus words.
...
The repeated loop is: **inspect a letter set, connect or arrange letters, submit candidates, and fill word slots**. Input is locked only during the shortest non-interruptible resolution phase. Cosmetic animation may continue after the logical state is ready, but skip/fast-forward must settle every object into the exact deterministic end state.

### Scoring and victory

Reward target completion and bonus vocabulary with locale-aware validation. Results show a component breakdown rather than one unexplained total. Store integers for score and simulation units; format values only in presentation. Ties use, in order: primary objective completion, fewer invalid actions, lower authoritative elapsed time, then stable session identifier.
...
The subject is the active playfield at near-tabletop to room scale, framed so state changes occupy most of the screen. The scene is a playful pantry where letter biscuits sit on a tray. Use an authored camera, original procedural geometry, restrained environmental storytelling, and a deterministic visual seed. The no-post-processing baseline must still communicate hierarchy, depth, selection, and state.
...
- Ship a browser distribution with `starhermit.txt` at its root, `name=Letter Pantry`, and `launch=index.html`. Keep source files, secrets, design documents, and source maps outside the uploaded distribution.
